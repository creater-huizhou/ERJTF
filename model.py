...
class CMT_block(nn.Module):
    def __init__(self, dim, num_heads, IRFFN_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.MLP = MLP(in_features=dim, hidden_features=IRFFN_hidden_dim, act_layer=act_layer, drop=drop)
...
class Residual_Project_Network(nn.Module):
    def __init__(self, input_dim, output_dim):
        super(Residual_Project_Network, self).__init__()
        self.conv1 = nn.Conv2d(input_dim, 4 * input_dim, 3, 1, 1)
        self.conv2 = nn.Conv2d(4 * input_dim, output_dim, 3, 1, 1)
...
class CMT_TransCNN(nn.Module):
    def __init__(self):
        ...
        self.CMT_encoder = CMTEncoders(d_model=dim, nhead=2, IRFFN_ratios=2, qkv_bias=True, qk_scale=None, dropout=0.1,
                                       activation=nn.GELU, qk_ratio=2, sr_ratios=4, normalize_before=False, num_encoder_layers=6,
                                       withCDP=True)
        self.CNN_decoder = CNNDecoders(256, 3, 'ln', 'relu', 'reflect')
        self.residual_project_network = Residual_Project_Network(3, 1)

    def forward(self, inputs, input_pos):
        patch_embedding = self.patch_to_embedding(inputs) # [4, 3, 256, 256] -> [4, 4096, 256]
        content = self.CMT_encoder(patch_embedding, input_pos)
        ...
        rec_img = self.CNN_decoder(content)

        residual = torch.sigmoid(rec_img - inputs)
        mask_pred = self.residual_project_network(residual)

        return rec_img, residual, mask_pred
...
input_pos = input_pos.unsqueeze(0).repeat(1, 1, 1, 1)
input_pos = input_pos.flatten(2).permute(0, 2, 1)
model = CMT_TransCNN()
input = torch.randn(2, 3, 128, 128)
flops, params = profile(model, inputs=(input, input_pos))
